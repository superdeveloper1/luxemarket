...
function AccountSummary({ isOpen, onClose, currentUser }) {
  ...
  const loadAccountData = () => {
    try {
      ...
      let watchlist = [];
      try {
        const stored = localStorage.getItem('luxemarket_watchlist');
        if (stored) {
          watchlist = JSON.parse(stored);
        }
      } catch (e) {
        console.warn('LocalStorage access failed (likely Incognito), defaulting to empty watchlist', e);
      }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} maxWidth="max-w-4xl" zIndex={60}>
      {/* Header */}
